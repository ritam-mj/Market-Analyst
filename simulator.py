...
class DigitalTwin:
    ...
    def _load_or_create_learner(self) -> SimulatorLearner:
        """Load learner state from disk, or create fresh instance."""
        if os.path.exists(self.LEARNER_STATE_FILE):
            try:
                with open(self.LEARNER_STATE_FILE, 'r') as f:
                    data = json.load(f)
                    learner = SimulatorLearner.from_dict(data)
                    print(f"[Loaded learner: {len(learner.history)} outcomes recorded]")
                    return learner
            except Exception as e:
                print(f"[Failed to load learner: {e}; creating fresh]")
    ...
    def save_learner(self) -> None:
        ...
        try:
            ...
            print(f"[Failed to save learner: {e}]")
